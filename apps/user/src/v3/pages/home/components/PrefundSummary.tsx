...
export const PrefundSummaryItem = ({ name, price }: { name: string; price: number }) => {
  return (
    <Row className='bg-white px-[20px] py-[16px]'>
      <Col span={14} className='flex pl-[16px] text-M/Regular text-grey-700'>
        <div className='mr-[10px] self-center'>
          <img src={abstract} width={20} />
        </div>
        <div>{name}</div>
      </Col>
      <Col span={10} className='text-right text-M/Medium text-grey-800'>
        {price}
      </Col>
    </Row>
  );
...
export const PrefundSummary = () => {
  return (
    <>
      <Row className='mt-[43px] rounded-tr-[8px] rounded-tl-[8px] bg-grey-200 py-[16px] px-[20px]'>
        <Col span={6} className='text-L/Medium text-grey-800'>
          전일매출
        </Col>
        <Col span={18} className='text-right text-L/Bold text-grey-800'>
          50,000,000원
        </Col>
      </Row>
      ...
      <Row className='border-y border-grey-200 bg-grey-100 px-[20px] py-[20px]'>
        <Col span={12} className='flex text-M/Regular text-grey-700'>
          <div className='mr-[10px] self-center'>
            <img src={equal} width={20} />
          </div>
          <div>선정산 금액</div>
        </Col>
        <Col span={12} className='text-right text-M/Medium text-grey-800'>
          45,000,000원
        </Col>
      </Row>
      <PrefundSummaryItem name='미래 정산금 상환' price={0} />
      <PrefundSummaryItem name='미래 정산 수수료' price={0} />
      <Row className='rounded-br-[8px] rounded-bl-[8px] bg-[#B36DEE] py-[16px] px-[20px]'>
